...
fieldConfigs.forEach((config)=>{
    ...
    if (config.labelText) label.textContent = config.labelText;
    let field;
    switch(config.tag){
        case "input":
    ...
    if (config.tag === "button") form.appendChild(field);
    else {
...
form.addEventListener("submit", (e)=>{
    ...
    const isValid = fieldConfigs.every((config)=>{
        ...
        let val;
        if (field.tagName === "SELECT") val = field.options[field.selectedIndex].text;
        else val = field.value.trim();
    ...
    fieldConfigs.forEach((config)=>{
        ...
        let fieldVal;
        if (field.tagName.toUpperCase() === "SELECT") fieldVal = field.options[field.selectedIndex].text;
        else fieldVal = field.value.trim();
...
tableBody.addEventListener("dblclick", (e)=>{
    ...
    if (!td || tableBody.querySelector("input.cell-input")) return;
    const colIndex = td.cellIndex;
    const config = fieldConfigs[colIndex];
    if (!config || config.tag === "button") return;
    ...
    const handleSave = ()=>{
        ...
        } else td.textContent = config.format ? config.format(val) : val;
    };
